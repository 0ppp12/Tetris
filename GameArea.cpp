	...
	if (e->timerId() == mTimerID)
	{
		...
		if (HitBottom())
		{
			...
			if (HitTop())
			{
                ...
                //看看该记录是否有必要记入字符串
                if(mScore>0){
                    QString name = QInputDialog::getText(this,"询问用户是否保留正常成绩","来将可留姓名");
//                    qDebug()<<name<<name.length();
                    if(name.length()>0){
                        query.exec(QString("insert into recode(date,time,name,score)values(date('now','localtime'),time('now','localtime'),'%1',%2)").arg(name).arg(mScore));
                    }
                }
